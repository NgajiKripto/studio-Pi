...
## 2026-06-06 - CSS/HTML Injection via Chart Component Styles
**Vulnerability:** In `src/components/ui/chart.tsx`, the `<ChartStyle>` component injects dynamic `key` and `color` configuration variables directly into a CSS string block rendered with `dangerouslySetInnerHTML`. An attacker controlling chart configuration could supply malicious payloads (e.g., `}</style><script>alert(1)</script>` for `color`) leading to Cross-Site Scripting (XSS).
**Learning:** The previous fix in `chart.tsx` successfully sanitized the `id` prop but left inner dynamic mapping values unsanitized, representing an incomplete mitigation pattern. When using `dangerouslySetInnerHTML` for style generation, *all* injected user-controlled values must be heavily sanitized.
**Prevention:** When using `dangerouslySetInnerHTML`, systematically audit every variable being interpolated. For CSS variable names (keys), sanitize non-alphanumeric characters `replace(/[^a-zA-Z0-9-_]/g, )`. For CSS values (colors), block break-out characters `replace(/[;{}<>"']/g, )`.
