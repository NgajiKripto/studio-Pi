...
## 2024-05-24 - Secure Cookie Flags on UI State
**Vulnerability:** UI state cookies (like sidebar state) were set without `SameSite=Lax; Secure` flags.
**Learning:** Even non-authentication cookies should be secured to enforce secure transmission and follow defense-in-depth principles.
**Prevention:** Always append `; SameSite=Lax; Secure` when using `document.cookie`, as noted in project memory.
