...
## 2024-05-28 - CSS/HTML Injection in shadcn/ui Chart Component
**Vulnerability:** The `ChartStyle` component dynamically generated `<style>` tags using `dangerouslySetInnerHTML` by directly interpolating `key` and `color` properties from user-provided configurations without sanitization.
**Learning:** Even internal UI libraries like shadcn/ui can introduce injection vulnerabilities if they generate raw HTML based on unsanitized dynamic configurations. The assumption that component configurations are entirely static or safe is flawed.
**Prevention:** Always sanitize dynamically interpolated strings used within `dangerouslySetInnerHTML`, even in internal component definitions. Use strict regex replacements like `/[^a-zA-Z0-9-_]/g` for keys and `/[;{}<>"']/g` for values to ensure only safe CSS is rendered.
