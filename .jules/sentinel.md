...
## 2025-05-24 - Missing CSS Injection Sanitization in Chart Container (color/key)
**Vulnerability:** The shadcn/ui chart component (`src/components/ui/chart.tsx`) was vulnerable to CSS and HTML injection through the `color` and `key` properties used in the `ChartStyle` component. These variables were directly used inside a `<style>` block rendered with `dangerouslySetInnerHTML`.
**Learning:** Even internal configuration mapping components like `ChartStyle` are vectors for XSS if they reflect dynamic or unsanitized values back into raw HTML string formats.
**Prevention:** Always explicitly sanitize variables injected into `<style>` blocks via `dangerouslySetInnerHTML`. Variables used as CSS variable names should be restricted to alphanumeric, hyphens and underscores (`replace(/[^a-zA-Z0-9-_]/g, "")`), and CSS values should strip characters capable of escaping the context or executing scripts (`replace(/[;{}<>"]/g, "")`).
