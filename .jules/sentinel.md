...
## 2025-05-24 - Missing CSS Injection Sanitization in Chart Container for Key and Color
**Vulnerability:** The shadcn/ui chart component (`src/components/ui/chart.tsx`) was vulnerable to CSS and HTML injection through the `key` and `color` properties used to dynamically construct CSS inside `dangerouslySetInnerHTML`. If an attacker gained control over these values, they could inject arbitrary HTML or malicious CSS payload.
**Learning:** `dangerouslySetInnerHTML` is commonly used in UI libraries, but passing unsanitized values like `id`, `key`, or `color` directly into its HTML string context is a critical XSS/CSS injection vector.
**Prevention:** Always explicitly sanitize user-provided values before injecting them into HTML/CSS string contexts. For CSS variables (`key`), enforce strict whitelisting of safe characters using regex (e.g., `replace(/[^a-zA-Z0-9-_]/g, "")`). For colors, remove dangerous characters like brackets, quotes, and HTML tag starters (e.g., `replace(/[;{}<>"']/g, "")`).
