...
## 2025-05-24 - Missing CSS/HTML Injection Sanitization on Config Iteration in Chart Style
**Vulnerability:** The shadcn/ui chart component (`src/components/ui/chart.tsx`) was vulnerable to CSS and HTML injection through the `key` and `color` variables inside the `ChartStyle`'s `dangerouslySetInnerHTML`. An attacker could inject malicious code by manipulating the `key` or `color` configuration values.
**Learning:** `dangerouslySetInnerHTML` is extremely risky when using interpolated strings. All dynamic variables in the template string passed to it need rigorous sanitization to strip control characters that could break out of the style context (e.g. `</style>`).
**Prevention:** Always sanitize dynamically interpolated strings inside `dangerouslySetInnerHTML` template strings using strict character whitelisting or blacklisting regexes (e.g. `replace(/[;{}<>"']/g, "")`).
