...
## 2025-05-24 - Missed Interpolation in dangerouslySetInnerHTML (ChartStyle)
**Vulnerability:** The dynamic chart component (`src/components/ui/chart.tsx`) was vulnerable to CSS and HTML injection because `config` key and color values were interpolated directly into the `ChartStyle` `<style dangerouslySetInnerHTML>` block without sanitization. An attacker controlling configuration values could inject arbitrary CSS or break out of the `<style>` tag to achieve XSS.
**Learning:** Sanitizing just the `id` prop of a container is not enough if child components also interpolate user-controlled values inside a `dangerouslySetInnerHTML` block. Each interpolated variable must be sanitized appropriately for its context.
**Prevention:** Always sanitize every interpolated value within a `dangerouslySetInnerHTML` context. Use regex patterns specific to the expected input, such as `/[^a-zA-Z0-9-_]/g` for CSS variable names and `/[;{}<>"]/g` to prevent escaping CSS values.
