...
## 2025-05-24 - Comprehensive CSS/HTML Injection in Chart Container
**Vulnerability:** Beyond the `id` prop, dynamic config `key` and `color` properties in the shadcn/ui chart component (`src/components/ui/chart.tsx`) were not fully sanitized when building CSS strings for `dangerouslySetInnerHTML`.
**Learning:** All string interpolations within a `dangerouslySetInnerHTML` `<style>` block must be treated as injection vectors, not just primary IDs. Valid CSS characters like colons (used in React unique IDs) must be handled properly (e.g., by quoting attribute selectors like `[data-chart="${id}"]`).
**Prevention:** Strictly sanitize all dynamic variables (`id`, `key`, `color`) using targeted allowlists (`/[^a-zA-Z0-9-_:]/g` for IDs, `/[^a-zA-Z0-9-_]/g` for keys, and `/[;{}<>"']/g` for colors) before embedding them in style templates.
