...
## 2024-05-18 - [Fix CSS/HTML Injection Vulnerability in Chart Component]
**Vulnerability:** Found a potential CSS/HTML injection via dangerouslySetInnerHTML in src/components/ui/chart.tsx when dealing with user-supplied color config keys.
**Learning:** The ChartComponent blindly accepts config `key` and `color` which could be manipulated via props.
**Prevention:** Apply rigorous Regex sanitization against unexpected characters, even if seemingly internal properties, when injecting via HTML.
