...
## 2026-05-05 - Insecure Cookie Configuration in Sidebar Component
**Vulnerability:** The sidebar state was being persisted in a cookie via `document.cookie = ...` without appending the `SameSite=Lax` and `Secure` security flags. This leaves the cookie susceptible to Cross-Site Request Forgery (CSRF) in cross-site requests and interception if transmitted over an unencrypted (HTTP) connection.
**Learning:** Direct assignments to `document.cookie` often overlook security flags because the browser's default behavior is less restrictive than modern security best practices require. Even non-sensitive UI state cookies should adopt these flags to maintain defense in depth.
**Prevention:** Whenever manually setting cookies via `document.cookie`, always append `; SameSite=Lax; Secure` to the string to enforce secure transmission and CSRF protection.
