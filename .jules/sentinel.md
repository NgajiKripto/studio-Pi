...
## 2026-05-02 - Insecure Cookie Handling in Client Components
**Vulnerability:** The application was setting a sidebar state cookie (`document.cookie = ...`) without the `SameSite` or `Secure` attributes. This could allow the cookie to be sent in cross-site requests (increasing CSRF risk) or transmitted over unencrypted connections if the user isn't using HTTPS.
**Learning:** Client-side cookie manipulation (like setting preferences in UI components) often overlooks security flags, focusing only on functionality (`path` and `max-age`).
**Prevention:** When manually setting cookies in the application (e.g., via `document.cookie`), always append security flags like `; SameSite=Lax; Secure` to protect against CSRF attacks and enforce secure transmission.
