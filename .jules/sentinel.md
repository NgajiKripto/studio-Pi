...
## 2023-10-27 - CSS Injection Vulnerability in Shadcn UI Chart Configuration Keys/Colors
**Vulnerability:** The shadcn/ui chart component (`src/components/ui/chart.tsx`) was vulnerable to CSS injection through the configuration object's `key` and `color` properties. The component used these values directly inside a `<style>` block rendered with `dangerouslySetInnerHTML`. If an attacker gained control over these values in `ChartConfig`, they could inject arbitrary CSS or break out of the style tag.
**Learning:** `dangerouslySetInnerHTML` combined with dynamic objects from props (even nested ones like configuration colors and keys) is a common vector for CSS and HTML injection attacks.
**Prevention:** Always explicitly sanitize user-provided or dynamically generated values like `key` and `color` before injecting them into HTML/CSS string contexts. For CSS variables and keys, enforce strict whitelisting of safe characters using regex (e.g., `replace(/[^a-zA-Z0-9-_]/g, "")` for keys and `replace(/[;{}<>"']/g, "")` for colors).
