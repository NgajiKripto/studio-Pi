...
## 2026-07-02 - [Sanitize dynamic inputs in CSS-in-JS components]
**Vulnerability:** Potential CSS/HTML injection via dangerouslySetInnerHTML in Shadcn/ui Chart component.
**Learning:** Dynamic keys and color values mapped directly into dangerouslySetInnerHTML without sanitization can be a vector for CSS injection. Furthermore, using user-controlled IDs in unquoted CSS attribute selectors (e.g., `[data-chart=${id}]`) can break styling if the ID contains valid React `useId()` colons but invalid unquoted CSS syntax.
**Prevention:** Always sanitize dynamically constructed CSS values using safe regex patterns (e.g., `/[;{}<>"']/g` for colors, `/[^a-zA-Z0-9-_]/g` for variables). Always quote dynamic values in CSS attribute selectors (e.g., `[data-chart="${id}"]`).
