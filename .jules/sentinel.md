...
## 2026-05-18 - Missing CSS/HTML Injection Sanitization on Chart Keys and Colors
**Vulnerability:** The shadcn/ui chart component (`src/components/ui/chart.tsx`) was vulnerable to CSS and HTML injection via the `key` and `color` configuration values used within the `ChartStyle` component. These values were directly interpolated into a `<style>` block rendered using `dangerouslySetInnerHTML`.
**Learning:** Even internal configuration objects that might be populated dynamically from user data or external sources must be sanitized when rendered within `dangerouslySetInnerHTML`. A malicious user could provide a key or color string containing `;{}<>"` to break out of the CSS rules and inject arbitrary CSS or HTML (like scripts).
**Prevention:** Always strictly sanitize user-provided configuration values (like keys and colors) before injecting them into HTML/CSS string contexts. Use regex (e.g., `replace(/[;{}<>"]/g, "")`) to remove potentially harmful characters.
