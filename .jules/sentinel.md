...
## 2025-05-24 - Missing CSS Injection Sanitization in Chart Configuration (key and color)
**Vulnerability:** The shadcn/ui chart component (`src/components/ui/chart.tsx`) was vulnerable to CSS injection through the dynamic configuration `key` and `color` variables. The component used these variables directly inside a `<style>` block rendered with `dangerouslySetInnerHTML`. An attacker controlling the chart configuration could inject malicious CSS rules.
**Learning:** Even if the `id` is sanitized, other dynamic parts of a template string used inside `dangerouslySetInnerHTML` must also be sanitized. In this case, `key` and `color` properties from `config` were injected raw.
**Prevention:** Always strictly sanitize all dynamic values interpolated into HTML/CSS string contexts within `dangerouslySetInnerHTML`. Use strict allowlists for keys (e.g., `/[^a-zA-Z0-9-_]/g`) and strip dangerous characters from colors (e.g., `/[;{}<>"']/g`).
