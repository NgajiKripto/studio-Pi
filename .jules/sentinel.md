...
## 2026-06-07 - CSS/HTML Injection Vulnerability in Chart Component
**Vulnerability:** The `src/components/ui/chart.tsx` component was vulnerable to CSS and HTML injection because dynamic `key` and `color` values were not sanitized before being injected into a `<style>` block using `dangerouslySetInnerHTML`.
**Learning:** Dynamic CSS values must be sanitized when using `dangerouslySetInnerHTML`. Malicious `color` or `key` config properties could allow injection of arbitrary scripts by breaking out of the `<style>` element or injecting unwanted CSS properties.
**Prevention:** Apply string replacement using regex to filter out special characters from CSS custom property keys (e.g., `/[^a-zA-Z0-9-_]/g`) and colors (e.g., `/[;{}<>"']/g`) before interpolation.
