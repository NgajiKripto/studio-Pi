...
## 2025-05-24 - Missing CSS Injection Sanitization in ChartStyle
**Vulnerability:** The shadcn/ui chart component (`src/components/ui/chart.tsx`) was vulnerable to CSS injection through the `key` and `color` variables inside `ChartStyle`. The component used these variables directly inside a `<style>` block rendered with `dangerouslySetInnerHTML`. If an attacker gained control over these variables, they could inject malicious CSS rules (e.g. `}; body { display: none;`).
**Learning:** `dangerouslySetInnerHTML` is commonly used in UI libraries for styling dynamic components, but passing unsanitized variables like `key` and `color` into these blocks is a common vector for CSS injection attacks.
**Prevention:** Always explicitly sanitize user-provided values before injecting them into HTML/CSS string contexts. For keys, enforce strict whitelisting of safe characters using regex (e.g., `replace(/[^a-zA-Z0-9-_]/g, "")`). For colors, remove potentially malicious characters (e.g., `replace(/[;{}<>"]/g, "")`).
