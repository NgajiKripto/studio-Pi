...
## 2026-05-14 - Missing CSS Injection Sanitization in Chart Configuration Properties
**Vulnerability:** The shadcn/ui chart component (`src/components/ui/chart.tsx`) was vulnerable to CSS and HTML injection through the `key` and `color` configuration properties. The component used these user-provided values directly inside a `<style>` block rendered with `dangerouslySetInnerHTML`. If an attacker gained control over the chart configuration, they could inject arbitrary HTML (e.g. `</style><script>alert(1)</script>`) or malicious CSS.
**Learning:** `dangerouslySetInnerHTML` is commonly used in UI libraries for styling dynamic components, but passing unsanitized properties from objects (like `key` and `color` in a config object) into these blocks is a common vector for XSS and CSS injection attacks.
**Prevention:** Always explicitly sanitize all user-provided or dynamic values before injecting them into HTML/CSS string contexts. For configuration keys and values, enforce strict whitelisting of safe characters using regex (e.g., `.replace(/[;{}<>"]/g, "")`).
