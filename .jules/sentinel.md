...
## 2025-05-24 - CSS/HTML Injection Risk in Dynamic Chart Configurations
**Vulnerability:** The chart configuration properties (`key` and `color`) inside `src/components/ui/chart.tsx` were rendered unsanitized within a `<style>` block using `dangerouslySetInnerHTML`. An attacker could inject malicious CSS rules or arbitrary HTML markup via these dynamic configurations.
**Learning:** Using `dangerouslySetInnerHTML` for styles is common but carries significant risks if the input variables are derived from configurable data. Sanitizing keys and values correctly prevents both CSS attribute injection and potential HTML execution.
**Prevention:** Always apply strict whitelisting to CSS variable names (e.g., `/[^a-zA-Z0-9-_]/g`) and CSS property values (e.g., removing `[;{}<>"]`) prior to inserting them into HTML string contexts.
