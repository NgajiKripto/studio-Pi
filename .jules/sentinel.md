...
## 2025-05-24 - Missing CSS Injection Sanitization in Dynamic Chart Styling Keys
**Vulnerability:** Even if the main container `id` is sanitized, dynamically generating `<style>` block contents (such as CSS variable names and values) within `dangerouslySetInnerHTML` using unsanitized object keys and properties leaves the component vulnerable to CSS and HTML injection.
**Learning:** Object keys and property values (like `key` and `color` in a config object) passed into template literals within `dangerouslySetInnerHTML` are just as dangerous as the main `id` attribute.
**Prevention:** Always sanitize every interpolated value inside a dynamically constructed string that is passed to `dangerouslySetInnerHTML`. Use strict allowlist regex replacements like `/[^a-zA-Z0-9-_]/g` for CSS keys/IDs and `/[;{}<>"]/g` for CSS values.
