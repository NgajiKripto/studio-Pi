...
## 2026-05-26 - Shadcn/ui Chart Component CSS/HTML Injection
**Vulnerability:** The `ChartStyle` component in Shadcn/ui charts dynamically creates a `<style>` block using `dangerouslySetInnerHTML`. It directly interpolates `key` and `color` variables from the chart configuration object into the CSS string. If an attacker controls the chart configuration values, they could inject malicious CSS or close the `<style>` tag to inject arbitrary HTML/scripts.
**Learning:** Even internal variables or object keys used in dynamic styling need to be sanitized when rendered via `dangerouslySetInnerHTML`, especially in components that might consume user-provided data.
**Prevention:** Sanitize keys to allow only CSS-safe characters (e.g., `/[^a-zA-Z0-9-_]/g`) and sanitize values to remove characters that can break out of a CSS rule or HTML tag (e.g., `/[;{}<>"]/g`).
