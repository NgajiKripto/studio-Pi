...
## 2026-05-29 - Missing CSS Injection Sanitization in Chart Container Configs
**Vulnerability:** The shadcn/ui chart component (src/components/ui/chart.tsx) was vulnerable to CSS and HTML injection through the configuration `key` and `color` properties. The component used these values directly inside a style block rendered with dangerouslySetInnerHTML. If an attacker gained control over these values, they could inject arbitrary HTML (e.g. closing style tag and adding a script tag) or malicious CSS.
**Learning:** dangerouslySetInnerHTML is commonly used in UI libraries for styling dynamic components, but passing unsanitized props into these blocks is a common vector for XSS and CSS injection attacks.
**Prevention:** Always explicitly sanitize user-provided values like `key`, and `color` before injecting them into HTML/CSS string contexts. For keys, enforce strict whitelisting of safe characters using regex. For colors, sanitize with regex to remove specific escape characters.
