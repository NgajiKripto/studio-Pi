...
## 2026-06-13 - Missing CSS Injection Sanitization in Chart Container (Keys & Colors)
**Vulnerability:** Similar to the `id` prop, the `key` and `color` properties derived from `chartConfig` in `src/components/ui/chart.tsx` were directly interpolated into a `<style>` block via `dangerouslySetInnerHTML`. If an attacker gained control over these config values, they could inject arbitrary HTML/CSS.
**Learning:** All dynamic values interpolated into a `<style>` block rendered with `dangerouslySetInnerHTML` must be sanitized, not just the parent `id`.
**Prevention:** Always enforce strict whitelisting of safe characters using regex for any dynamic value injected into CSS contexts. For keys, allow `a-zA-Z0-9-_`. For colors, remove dangerous characters like `;{}<>\"'`.
