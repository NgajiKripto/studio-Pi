...
## 2025-05-24 - Missing Secure Flags in Cookies
**Vulnerability:** The `document.cookie` assignment in `src/components/ui/sidebar.tsx` was missing the `SameSite=Lax` and `Secure` flags. This made the cookie vulnerable to Cross-Site Request Forgery (CSRF) and allowed it to be transmitted over insecure connections, potentially exposing it to Man-in-the-Middle (MITM) attacks.
**Learning:** Even non-authentication cookies like UI state (e.g. sidebar open state) should use secure flags to adhere to defense-in-depth principles.
**Prevention:** Whenever manually setting cookies via `document.cookie`, always append `; SameSite=Lax; Secure` (or `SameSite=Strict` if appropriate) to ensure secure transmission and protection from cross-site requests.
