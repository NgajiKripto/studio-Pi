...
## 2026-05-25 - CSS/HTML Injection Vulnerability in Chart Configuration
**Vulnerability:** The `ChartStyle` component in `shadcn/ui` charts dynamically generated CSS variables from chart configuration `key` and `color` properties and injected them directly into a `<style>` block using `dangerouslySetInnerHTML`. If a user could control these chart configurations, they could inject malicious characters (e.g., `}</style><script>alert(1)</script>`) to break out of the CSS context and execute arbitrary HTML/JavaScript (XSS) or malicious CSS.
**Learning:** When using `dangerouslySetInnerHTML` to construct `<style>` blocks dynamically, any user-controllable input (even seemingly benign data like colors or keys) can be an injection vector if not properly sanitized.
**Prevention:** Always sanitize dynamic values before injecting them into CSS/HTML contexts. For CSS variables, sanitize keys by restricting to alphanumerics, hyphens, and underscores (`replace(/[^a-zA-Z0-9-_]/g, "")`). Sanitize colors by removing characters that could terminate CSS rules or HTML tags (`replace(/[;{}<>"]/g, "")`).
