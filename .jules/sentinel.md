...
## 2026-05-06 - Insecure Cookie Configuration
**Vulnerability:** The application was setting a sidebar state cookie without `SameSite` or `Secure` attributes (`document.cookie = sidebar_state=val; path=/; max-age=...`). This omission could expose the cookie to cross-site transmission or allow it to be sent over unencrypted connections. While this specific cookie may not have been highly sensitive, failing to secure cookies is a poor security posture and violates defense-in-depth principles.
**Learning:** Even low-value cookies should be secured to reduce the overall attack surface and enforce secure-by-default behavior across the codebase.
**Prevention:** Always append `; SameSite=Lax; Secure` when manually setting cookies in the client-side code via `document.cookie`.
