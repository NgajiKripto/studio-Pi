...
## 2025-02-28 - CSS/HTML Injection via Chart Configs
**Vulnerability:** Shadcn UI `ChartStyle` component passes un-sanitized dynamic configuration keys (`id`, `key`, `color`) directly into a `<style dangerouslySetInnerHTML>` block.
**Learning:** This is a common pattern in component libraries relying on string interpolation for internal styles. Even generic UI configurations from external contexts can become vectors for CSS and HTML injection.
**Prevention:** Always strictly sanitize dynamic inputs feeding into `<style>` or `dangerouslySetInnerHTML`. Ensure regex replacements account for all valid CSS identifier characters (like `_` and `:`) while quoting the attribute values (e.g., `[data-chart="${id}"]`) to avoid syntactical breakage.
