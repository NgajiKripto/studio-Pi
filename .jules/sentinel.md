...
## 2025-05-24 - Missing Secure Flags in Manual Cookie Assignment
**Vulnerability:** The application manually set a cookie for sidebar state using `document.cookie` without including `SameSite=Lax` or `Secure` flags. This left the cookie vulnerable to Cross-Site Request Forgery (CSRF) and transmission over insecure connections.
**Learning:** When directly manipulating `document.cookie`, security flags are not automatically appended as they might be when using standard cookie libraries or frameworks.
**Prevention:** Always append security flags like `; SameSite=Lax; Secure` when manually setting cookies in the application (e.g., via `document.cookie`) to protect against CSRF attacks and enforce secure transmission.
